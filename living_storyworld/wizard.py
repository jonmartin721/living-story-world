...
def run_setup_wizard() -> None:
    ...
    # Defaults
    style = _pick(
        "Pick a default art style:",
        list(STYLE_PACKS.keys()),
        default=s.default_style_pack,
    )
    preset = _pick(
        "Pick a default story preset:", list(PRESETS.keys()), default=s.default_preset
    )
...
def run_world_wizard() -> None:
    ...
    theme = (
        input("One-line theme (e.g., A city of drowned archives): ").strip()
    ...
    # User selection (not currently used)
    _pick("Story preset:", list(PRESETS.keys()), default=s.default_preset)
    slug = slugify(title)
    ...
    # Offer first chapter
    go = (
        input("Generate your first chapter now? [Y/n]: ").strip().lower() or "y"
    ).startswith("y")
    if not go:
        return
    ...
    # Offer viewer build
    build = (
        input("Build the simple HTML viewer now? [Y/n]: ").strip().lower() or "y"
    ).startswith("y")
    if build:
        import argparse
